...
async function getWordPressPosts() {
  try {
    const response = await fetch('https://blog.gammaiptv.online/wp-json/wp/v2/posts?per_page=100', {
...
export async function GET() {
  ...
  const postEntries = posts.length > 0 
    ? posts.map((post: any) => `
  <url>
    <loc>https://gammaiptv.online/blog/${post.slug}</loc>
  ...
  <url>
    <loc>https://gammaiptv.online/blog</loc>
