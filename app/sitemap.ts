...
async function getWordPressPosts() {
  try {
    const response = await fetch('https://blog.serviceproiptv.com/wp-json/wp/v2/posts?per_page=100', {
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPages = [
    ...
    {
      url: 'https://serviceproiptv.com/pricing',
      lastModified: currentDate,
    ...
    {
      url: 'https://serviceproiptv.com/contact',
      lastModified: currentDate,
      changeFrequency: 'monthly' as const,
  ...
  const blogPages = posts.map((post: any) => ({
    url: `https://serviceproiptv.com/blog/${post.slug}`,
